<template>
  <div class="min-h-screen w-full flex flex-col items-center justify-center bg-white overflow-hidden py-12 px-4 sm:px-4 lg:px-5">
    <div class="w-full max-w-xl flex flex-col justify-center px-4 sm:px-8 py-12 bg-white sm:rounded-[2rem] relative z-10 my-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
      ...
      <div class="mb-10 text-center flex flex-col items-center">
        ...
        <h1 class="text-3xl font-extrabold text-gray-900 mb-2 tracking-tight">Complete your profile</h1>
        <p class="text-gray-500 text-base">Just a few more details to get you started.</p>
      </div>
      ...
      <form @submit.prevent="nextStep" class="space-y-6 w-full">
        ...
        <div v-if="currentStep === 1" class="space-y-4 animate-fade-in">
          ...
          <div class="space-y-2">
            ...
            <input v-model="form.phone" type="tel" required placeholder="+234..." class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-[#FF5C1A] focus:ring-4 focus:ring-[#FF5C1A]/10 transition-all text-base outline-none" />
          </div>
          <div class="space-y-2">
            <label class="text-sm font-bold text-gray-700">Date of Birth</label>
            <input v-model="form.dateOfBirth" type="date" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-[#FF5C1A] focus:ring-4 focus:ring-[#FF5C1A]/10 transition-all text-base outline-none" />
          </div>
          <div class="space-y-2">
            <label class="text-sm font-bold text-gray-700">Gender</label>
            <select v-model="form.gender" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-[#FF5C1A] focus:ring-4 focus:ring-[#FF5C1A]/10 transition-all text-base outline-none appearance-none">
        ...
        <div class="flex items-center gap-4 mt-8 pt-6 border-t border-gray-100">
          <button v-if="currentStep > 1" type="button" @click="prevStep" :disabled="saving" class="flex-1 py-3.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-bold text-base transition-colors disabled:opacity-50">
            Back
          </button>
          
          <button v-if="currentStep < 3" type="submit" class="flex-1 py-3.5 bg-[#FF5C1A] hover:bg-[#E54D12] text-white rounded-xl font-bold text-base transition-colors">
            Continue
          </button>
          
          <button v-if="currentStep === 3" type="button" @click="handleSave" :disabled="saving" class="flex-1 py-3.5 bg-[#008950] hover:bg-[#007040] text-white rounded-xl font-bold text-base transition-colors flex items-center justify-center gap-2">
