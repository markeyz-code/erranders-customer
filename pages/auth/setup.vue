    ...
    <div class="w-full max-w-xl flex flex-col justify-center px-4 sm:px-8 py-8 bg-white sm:rounded-[2rem] relative z-10 my-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
      ...
      <form @submit.prevent="nextStep" class="space-y-6 w-full">
        ...
        <div class="flex items-center gap-4 mt-5 pt-6 border-t border-gray-100">
          ...
          <button v-if="currentStep < 3" type="submit" class="flex-1 py-2.5 bg-[#FF5C1A] hover:bg-[#E54D12] text-white rounded-xl font-bold text-sm transition-colors">
            Continue
          </button>
          
          <button v-if="currentStep === 3" type="button" @click="handleSave" :disabled="saving" class="flex-1 py-2.5 bg-[#008950] hover:bg-[#007040] text-white rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2">
